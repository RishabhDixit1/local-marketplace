...
class LocalityProvidersScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<LocalityProvidersScreen> createState() =>
      _LocalityProvidersScreenState();
}

class _LocalityProvidersScreenState
    extends ConsumerState<LocalityProvidersScreen> {
  @override
  Widget build(BuildContext context) {
    final providersAsync = ref.watch(
      _localityProvidersProvider(widget.localityId),
    );
          ...
          children: [
            Padding(
              ...
              child: Container(
                ...
                child: AiPromptBar(
                  placeholder:
                      'Ask about services in ${widget.localityName}...',
                ),
              ),
            ),
            Expanded(
                ...
                data: (providers) {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final cleanedName = AppFormatters.cleanPersonName(
                        p['name'] as String? ?? p['full_name'] as String?,
                      );
                      ...
                      final location =
                          (p['location'] as String? ??
                              p['locality_name'] as String?) ??
                          '';
                      final avatarUrl = p['avatar_url'] as String?;
                      final bio = p['bio'] as String?;
                      final completedJobs = p['completed_jobs'];
                      final jobs = completedJobs is num
                          ? completedJobs.toInt()
                          : 0;
                      final responseMinutes =
                          p['response_minutes'] ?? p['response_time_minutes'];
                      final respMin = responseMinutes is num
                          ? responseMinutes.toInt()
                          : null;
                      final trustScore = p['trust_score'];
                      final score = trustScore is num
                          ? trustScore.toDouble()
                          : null;
