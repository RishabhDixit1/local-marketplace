...
export default function ChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-7.5rem)] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="flex h-full">
        ...
        >
          <div className="border-b border-slate-200/80 px-5 pb-5 pt-5">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-indigo-600">Inbox</p>
                <h2 className="mt-1 text-xl font-semibold text-slate-900">Messages</h2>
                <p className="mt-1 text-xs text-slate-500">Open a thread and continue local coordination in real time.</p>
              </div>
              <span
        ...
        <section className={`flex-1 flex-col ${selectedChat ? "flex" : "hidden md:flex"}`}>
          ...
          {!selectedChat ? (
            <div className="flex h-full items-center justify-center overflow-y-auto p-6 md:p-10">
              <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-7">
                <h3 className="text-xl font-semibold text-slate-900">Choose a conversation</h3>
                <p className="mt-1 text-sm text-slate-600">
                  {conversations.length
                    ? "Pick a thread from the inbox to start messaging."
                    : "No conversations yet. Start one from Posts or People."}
                </p>
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  {conversations.slice(0, 3).map((conversation) => (
                    <button
                      ...
                      className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition hover:border-indigo-300 hover:text-indigo-700"
                    >
                      {conversation.name}
                    </button>
                  ))}
            ...
            <>
              <header className="border-b border-slate-200/80 bg-white/90 px-4 py-4 backdrop-blur-xl sm:px-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  >
                    <span className={`h-2 w-2 rounded-full ${connectedChannels >= 2 ? "bg-emerald-500" : "bg-amber-500"}`} />
                    {connectedChannels >= 2 ? "Realtime healthy" : "Realtime recovering"}
                  </span>
                </div>
              </header>
