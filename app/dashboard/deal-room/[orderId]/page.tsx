...
export default function DealRoomPage() {
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-10">
        <div className="flex items-center justify-center gap-3 py-20 text-sm text-[var(--ink-500)]">
  ...
  if (error || orderNotFound) {
    return (
      <div className="mx-auto max-w-4xl px-4 py-10">
        <div className="flex flex-col items-center gap-4 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] px-6 py-16 text-center shadow-sm">
          <p className="text-base font-semibold text-[var(--ink-950)]">{error || "Deal room not found."}</p>
          <p className="text-sm text-[var(--ink-500)]">The order may have been deleted or you may not have access.</p>
          <button
            type="button"
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6">
      {/* Breadcrumb */}
      <nav className="mb-4 flex items-center gap-2 text-xs text-[var(--ink-500)]">
        <button
          type="button"
          onClick={() => router.push("/dashboard")}
          className="hover:text-[var(--ink-700)]"
        >
          Dashboard
        </button>
        <span>/</span>
        ...
        <span>/</span>
        <span className="font-medium text-[var(--ink-700)]">{orderId?.slice(0, 8)}</span>
      </nav>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={handleBack}
            className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] text-[var(--ink-500)] transition hover:border-[var(--border-strong)]"
          >
            <ArrowLeft className="h-4 w-4" />
          </button>
          <div>
            <h1 className="text-lg font-bold text-[var(--ink-950)]">Deal Room</h1>
            <p className="text-xs text-[var(--ink-500)]">Quote, scope, and job workspace</p>
          </div>
        </div>
        {helpRequestId && (
          <button
