...
function ProviderSkeleton() {
  return (
    <div className="group flex min-w-[200px] sm:min-w-[240px] max-w-[280px] shrink-0 flex-col gap-2 rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-4">
    ...
    <div className="group flex min-w-[200px] sm:min-w-[250px] max-w-[290px] shrink-0 flex-col gap-2.5 overflow-hidden rounded-2xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] p-4 transition hover:border-[var(--brand-300)] hover:shadow-lg hover:shadow-slate-200/50">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <p className="truncate text-sm font-bold text-[var(--ink-950)]">{provider.name}</p>
            {provider.verified && (
          ...
          <p className="truncate text-xs text-[var(--ink-500)]">{provider.location || "Local area"}</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 text-xs">
        {provider.avgRating ? (
          ...
          <span className="inline-flex items-center gap-1 rounded-full bg-[var(--surface-soft)] px-2 py-0.5 text-[var(--ink-500)]">
            <Star className="h-3 w-3 text-[var(--ink-500)]" />
            No reviews
          </span>
        )}
        {provider.completedJobs > 0 && (
          <span className="inline-flex items-center gap-1 rounded-full bg-[var(--surface-soft)] px-2 py-0.5 text-[var(--ink-700)]">
      ...
      {provider.listings.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {provider.listings.slice(0, 3).map((l) => (
            ...
            >
              <Store className="h-2.5 w-2.5 text-[var(--ink-500)]" />
              {l.title}
              {l.price != null && (
          ...
          {provider.listings.length > 3 && (
            <span className="inline-flex items-center rounded-lg bg-[var(--surface-soft)] px-2 py-0.5 text-[10px] font-medium text-[var(--ink-500)]">
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1.5 border-t border-[var(--surface-border)] pt-2.5">
        <Link
        ...
        {buildPublicProfilePath(provider) && (
          <Link
            ...
            className="inline-flex items-center gap-1 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] px-3 min-h-9 py-2 text-xs font-semibold text-[var(--ink-700)] transition hover:border-[var(--brand-300)] hover:bg-[var(--surface-soft)]"
          >
        ...
        {isConnected ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-1 rounded-xl border border-[var(--surface-border)] bg-[var(--surface-elevated)] px-3 min-h-9 py-2 text-xs font-semibold text-[var(--ink-700)] transition hover:border-[var(--brand-300)] hover:bg-[var(--surface-soft)] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isConnecting ? (
