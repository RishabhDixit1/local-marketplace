    ...
    <div className="mx-auto max-w-4xl space-y-4 p-4 sm:p-6">
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5">
        ...
        <div className="mt-4 flex gap-2">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-7 w-20 animate-pulse rounded-xl bg-slate-100" />
          ))}
        </div>
        <div className="mt-4 flex gap-2">
          <div className="h-9 w-28 animate-pulse rounded-xl bg-slate-200" />
          <div className="h-9 w-36 animate-pulse rounded-xl border border-slate-100 bg-white" />
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="ml-auto h-8 w-24 animate-pulse rounded-xl bg-slate-100" />
      </div>

      {/* Feed skeleton */}
      <div className="grid gap-3 sm:grid-cols-2">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="overflow-hidden rounded-[1.4rem] border border-slate-200 bg-white p-3.5 shadow-sm sm:rounded-[1.6rem] sm:p-4">
            <div className="flex items-center gap-3">
