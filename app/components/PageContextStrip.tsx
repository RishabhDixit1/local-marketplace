...
export default function PageContextStrip({ label, description, action, switchAction }: PageContextStripProps) {
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-[var(--surface-border)] bg-white/92 px-4 py-3 shadow-sm backdrop-blur-sm sm:flex-row sm:items-center sm:justify-between sm:px-5">
      <div className="min-w-0 sm:flex-1">
        <span className="mr-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--brand-700)]">
          {label}
        </span>
        <span className="text-xs text-[var(--ink-500)] sm:text-sm">{description}</span>
      </div>
      ...
      <div className="flex w-full shrink-0 flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3">
        {switchAction && (
          <Link
            ...
            className="inline-flex min-h-10 items-center justify-center rounded-full border border-[var(--surface-border)] px-3.5 py-2 text-xs font-semibold text-[var(--ink-700)] transition hover:border-[var(--border-strong)] hover:text-[var(--ink-950)] sm:min-h-0 sm:border-transparent sm:p-0 sm:font-medium sm:underline-offset-2 sm:hover:underline"
          >
            {switchAction.label}
          </Link>
        )}
